...
pub fn miner_fees_are_valid(
    ...
    // Coinbase transaction can still have a burn amount
...
pub fn miner_fees_are_valid(
    ...
    // Verify that the burn amount is at least the minimum required amount (ZIP-235).
    // #[cfg(zcash_unstable = "nsm")]
    // if network_upgrade == NetworkUpgrade::ZFuture {
    //     let minimum_zsf_deposit = ((block_miner_fees * 6).unwrap() / 10).unwrap();

    //     if burn_amount < minimum_zsf_deposit {
