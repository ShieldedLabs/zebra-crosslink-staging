...
pub trait GetBlockTemplateRpc {
    ...
    ///
    /// - `burn_amount`: (numeric, optional) The NSM burn_amount fiel
    /// # Notes
    ///
    ...
    fn generate(
        &self,
        num_blocks: u32,
        // the burn_amount field should be hidden behind zcash_unstable flag, but it doesn't compile due to the rpc macro
    ...
    AddressBook: AddressBookPeers + Clone + Send + Sync + 'static,
{
    ...
    fn generate(
        ...
        async move {
            ...
            let params = None;
            #[cfg(zcash_unstable = "nsm")]
            let params = if let Some(burn_amount) = burn_amount {
            ...
            for _ in 0..num_blocks {
                let block_template = rpc.get_block_template(params.clone()).await.map_server_error()?;
