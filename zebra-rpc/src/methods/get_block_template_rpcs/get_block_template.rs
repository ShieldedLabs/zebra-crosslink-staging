    ...
    if like_zcashd {
        #[cfg(zcash_unstable = "nsm")]
        {
            ...
            if network_upgrade < NetworkUpgrade::ZFuture {
                return Transaction::new_v4_coinbase(network, height, outputs, like_zcashd, extra_coinbase_data).into();
            } else {
                return Transaction::new_zfuture_coinbase(network, height, outputs, extra_coinbase_data, like_zcashd, burn_amount).into();
            }
        }
        ...
        #[cfg(zcash_unstable = "nsm")]
        {
            let network_upgrade = NetworkUpgrade::current(network, height);
            if network_upgrade < NetworkUpgrade::ZFuture {
                return Transaction::new_v5_coinbase(network, height, outputs, extra_coinbase_data).into();
            } else {
                return Transaction::new_zfuture_coinbase(network, height, outputs, extra_coinbase_data, like_zcashd, burn_amount).into();
            }
        }
